...
gerap <- \(indicador="objetivo1") {
  ...
  baseind <-
    dbGetQuery(
      ...
      WHERE orig_name LIKE '%",indicador,"%' AND EXTRACT('Month' FROM refdate) = 12"))

  data.table::setDT(baseind)
  baseind <- baseind[ano>2012]

  saveRDS(baseind,paste0("coleta/cache/indicadores_atualizados/",indicador,".rds"))
  basefplanilha <-
    baseind|>tidyr::pivot_wider(names_from=c(ano,orig_name),names_sep = "_",values_from=valor)


    writexl::write_xlsx(basefplanilha,paste0('coleta/cache/indicadores_atualizados/',indicador,".xlsx"))

}

indicadores_planilhas <- unique(gsub("(?<!vo)\\d","\\1",unique(gsub("_[^_]+$","",dbGetQuery(
  con , "SELECT DISTINCT(orig_name) from mdata")$orig_name)),perl=TRUE))
