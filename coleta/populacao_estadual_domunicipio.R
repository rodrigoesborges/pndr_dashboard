...
###Cria indicador
dbdbase <- populacao_estadual_domunicipio <- dbdbase|>
                            dplyr::group_by(refdate,estado)|> dplyr::mutate(datasus_popmun=somasna(datasus_popmun)) |>
                     dplyr::rename(setNames(c('datasus_popmun'), c('a'))) |>
                dplyr::transmute(populacao_estadual_domunicipio = a,refdate,local_id)|> dplyr::ungroup()
