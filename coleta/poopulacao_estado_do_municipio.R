...
###Cria indicador
dbdbase <- poopulacao_estado_do_municipio <- dbdbase|>
                            dplyr::group_by(refdate,estado)|> dplyr::mutate(datasus_popmun= somasna(datasus_popmun))|>dplyr::ungroup() |>
                     dplyr::rename(setNames(c('datasus_popmun'), c('a'))) |>
                dplyr::transmute(poopulacao_estado_do_municipio = a,refdate,local_id)|> dplyr::ungroup()
